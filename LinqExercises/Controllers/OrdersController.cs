...
namespace LinqExercises.Controllers
{
    public class OrdersController : ApiController
    {
        ...
        {
            //throw new NotImplementedException("Write a query to return all orders with required 
            //dates between the given start date and the given end date with freight under 100 units.");
            var resultSet = _db.Orders
                   .Where(currentOrder => currentOrder.RequiredDate >= startDate
                            && currentOrder.RequiredDate <= endDate
                            && currentOrder.Freight < 100);
        ...
        public IHttpActionResult PurchaseReport()
        {
            ...
            ");*/
            var resultSet = _db.Products.Select(currentProduct => new
               {
                   Product = currentProduct.ProductName,
                   QuantityPurchased = currentProduct.UnitsOnOrder
               })
